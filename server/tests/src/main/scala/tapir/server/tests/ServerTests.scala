...
//  }
  ...
  testServer(in_value_class_out_string_constraints)((wrapper: IntWrapper) => pureResult(s"count: ${wrapper.value}".asRight[Unit])) {
    baseUri =>
      sttp.get(uri"$baseUri?count=100").send().map(_.body shouldBe Right("count: 100")) *>
        sttp.get(uri"$baseUri?count=1").send().map(_.body shouldBe Left("Invalid value for: query parameter count"))
  }
